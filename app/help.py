...
def display_cli_help():
    ...
    Terminal.inst().message("")
    Terminal.inst().message("Identities are defined in config/identities.json located at .siis local data.")
    Terminal.inst().message("Profiles must be defined in config/profiles/*.json directory or sub-directories located at .siis local data.")
    Terminal.inst().message("")
    Terminal.inst().message("To exit during execution uses : --quit <save> <term>. Optional save to database option. Optional terminate strategy before exit.")
    Terminal.inst().message("")
    ...
    Terminal.inst().message("    If omitted use whole data set (take care).")
    ...
    Terminal.inst().message("  --training Allow training sub process to be called during a backtest.")
    Terminal.inst().message("")
    ...
    Terminal.inst().message("  --install-market Used only with the fetcher tool, to install the fake market data info to database without trying to fetch anything from the exchange.")
    Terminal.inst().message("  --spec=<specific-option> Specific fetcher option (example STOCK for alphavantage.co fetcher to fetch a stock market).")
    ...
    Terminal.inst().message("  --sync Synchronize the market data from the broker.")
    Terminal.inst().message("    Specify --broker and --market.")
    ...
    Terminal.inst().message("")
    Terminal.inst().message("During usage press ':h<ENTER>' or ':help<ENTER>' to get interactive commands help.")
    Terminal.inst().message("To exit program press : '--quit<ENTER>'. Two optionals parameters are possible with this command :")
    Terminal.inst().message("    - save To save to user data to database before exit.")
    Terminal.inst().message("    - terminate To call the terminate method of the strategy to cancel any pending trades.")
    Terminal.inst().message("")
