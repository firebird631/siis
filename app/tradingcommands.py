...
class UserExportCommand(Command):
    SUMMARY = "to export actives trades or history at current state in a JSON or CSV file"
    HELP = (
        "param1: <market-id> market-id or any if not specified (optional)",
        "param2: <active|history|closed|alert|region> Active trade, history trades, alerts or region "
        "(closed means for history) (optional) (default history)",
    ...
    CHOICES = ("active", "history", "closed", "alert", "region", "csv", "json", "pending")
...
class UserImportCommand(Command):
    SUMMARY = "to import actives trades, alerts or regions from a JSON file, previously generated by export command"
    HELP = (
        "param1: <active|alert|region> Active trade, history trades, alerts or region",
        "param2: <filename> Specify the full-path filename (optional) (default /tmp/siis_trades.json)"
    )

    CHOICES = ("active", "history", "closed", "alert", "region")
    ...
    def execute(self, args):
        # ie: ":import active /tmp/siis_trades.json"
        dataset = ""
        filename = ""
...
def register_trading_commands(commands_handler, watcher_service, trader_service, strategy_service,
    ...
    commands_handler.register(UserImportCommand(strategy_service))
