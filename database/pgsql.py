    ...
    def setup_userdata_sql(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS asset(
                ...
                quantity VARCHAR(32) NOT NULL, price VARCHAR(32) NOT NULL, quote_symbol VARCHAR(32) NOT NULL, UNIQUE(broker_id, asset_id))""")
    ...
    def setup_ohlc_sql(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS ohlc(
                ...
                volume VARCHAR(48) NOT NULL, UNIQUE(broker_id, market_id, timestamp, timeframe))""")
        ...
        try:
            ...
            for mi in mis:
                cursor.execute("""SELECT symbol, expiry, timestamp,
                                    ...
                                    margin_factor, market_type, unit_type, bid, ofr,
        ...
        try:
            ...
            for ut in uti:
                ...
                cursor.execute("""
                    ...
                    ON CONFLICT (broker_id, market_id,  appliance_id, trade_id) DO UPDATE SET 
