...
class PgSql(Database):
    ...
    def setup_event_sql(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS economic_event(
                ...
                UNIQUE(code, date, country, currency))""")
                ...
                for mi in mki:
                    ...
                    cursor.execute("""INSERT INTO market(broker_id, market_id, symbol,
                                    ...
                                    ON CONFLICT (broker_id, market_id) DO UPDATE SET symbol = EXCLUDED.symbol,
                                        ...
                                        settlement = EXCLUDED.settlement, settlement_display = EXCLUDED.settlement_display, settlement_precision = EXCLUDED.settlement_precision,
    ...
    def process_economic_events(self):
        ...
        if self._pending_economic_event_insert and (self._pending_economic_event_select or (len(self._pending_economic_event_insert) >= 500) or (
                ...
                try:
                    ...
                    for ev in evt:
                        if (ev.code, ev.date, ev.country, ev.currency) not in data:
                            elts.append("('%s', '%s', '%s', '%s', '%s', '%s', %i, '%s', '%s', '%s', %i, %i)" % (
                                ev.code, ev.date.strftime('%Y-%m-%dT%H:%M'), ev.country, ev.currency, ev.title.replace("'", "''"), ev.reference, ev.level, ev.previous, ev.actual, ev.forecast, ev.actual_meaning, ev.previous_meaning))
                    ...
                    query = ' '.join((
                                     ...
                                     ','.join(elts),
                                     "ON CONFLICT (code, date, country, currency) DO UPDATE SET title = EXCLUDED.title, reference = EXCLUDED.reference, level = EXCLUDED.level, previous = EXCLUDED.previous, actual = EXCLUDED.actual, forecast = EXCLUDED.forecast, actual_meaning = EXCLUDED.actual_meaning, previous_meaning = EXCLUDED.previous_meaning"))
