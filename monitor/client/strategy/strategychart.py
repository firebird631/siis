...
from monitor.streamable import Streamable, StreamMemberSerie, StreamMemberFloatSerie, StreamMemberFloatScatter, \
	StreamMemberFloatBarSerie, StreamMemberCandleSerie
...
class StrategyChartCandleSerie(StrategyChartBase):

	CHART_CANDLE_SERIE = 4

	def __init__(self, name, width):
		super().__init__(name, StrategyChartCandleSerie.CHART_CANDLE_SERIE)
...
class StrategyChart(object):
	...
	def on_chart(self, data):
		...
		if self.chart:
			...
			elif data['t'] == StreamMemberCandleSerie.TYPE_CANDLE_SERIE:
				if self.begin and data['b'] == self.begin:
					# append for the current serie timestamp
					if not self.candles:
						self.candles = StrategyChartCandleSerie(data['n'], self.window)
