...
Base on original work from https://github.com/kroitor/asciichart/tree/master/asciichartpy
"""
...
def plot(series, cfg=None):
    ...
    def clamp(_n):
        return min(max(_n, minimum), maximum)

    def scaled(_y):
        return int(round(clamp(_y) * ratio) - min2)
