...
def reduce_position(trader, market, position, close_exec_price, reduce_quantity):
    ...
    order.quantity = min(reduce_quantity, position.quantity)
    order.leverage = position.leverage
    ...
    if position_gain_loss != 0.0 and realized_position_cost > 0.0:
        ...
        # display only for debug
        if position_gain_loss > 0.0:
            Terminal.inst().high("Reduce profitable position with %.2f on %s (%.2fpips) (%.2f%%) at %s" % (
                position_gain_loss, order.symbol, delta_price/one_pip_means, gain_loss_rate*100.0,
                market.format_price(close_exec_price)), view='debug')
        elif position_gain_loss < 0.0:
            Terminal.inst().low("Reduce loosing position with %.2f on %s (%.2fpips) (%.2f%%) at %s" % (
                position_gain_loss, order.symbol, delta_price/one_pip_means, gain_loss_rate*100.0,
                market.format_price(close_exec_price)), view='debug')
    else:
        gain_loss_rate = 0.0
