...
class OneBrokerTrader(Trader):
	...
	@todo update order and position throught WS API
	@todo cancel_order
	...
	def tradeable(self, market_id):
		...
		"""
		return self._watcher.connected and market_id in self._markets and self._markets[market_id].is_open
	...
	def create_order(self, order):
		if not self.has_market(order.symbol):
			Terminal.inst().error("Trader %s does not support market %s in order %s !" % (self.name, order.symbol, order.order_id))
			return False
		...
		leverage = self.clamp_symbol_leverage(order.symbol, order.leverage)
